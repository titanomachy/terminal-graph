...
Focused programs under `examples/` cover bars, line charts, scatter and XY
charts, static frames, streaming displays, sparklines, surfaces, contours, and
multiplot grids. `all_graphs.nim` provides the finite aggregate showcase.

`LiveDashboard` provides resize-safe full-screen lifecycle management for
arbitrary composed frames. `LiveGraph` and `LiveLineGraph` retain their focused
data and rendering APIs and can supply frames through `renderFrame()`.
